...
## Режим разработчика
Эта возможность предназначена для разработчиков backend, которые хотят тестировать своё API с реальным iOS-приложением, не занимаясь сборкой приложения самостоятельно. Достаточно установить приложение через TestFlight.

Чтобы включить режим выбора backend-сервера, укажите `provListURL` в конфигурации приложения — URL-адрес файла со списком доступных backend-серверов. В сборках Debug и TestFlight экран выбора backend будет отображаться при каждом запуске, даже если задан `defaultBackendURL`. В App Store сборке этот экран скрыт, и приложение использует сервер по умолчанию.
