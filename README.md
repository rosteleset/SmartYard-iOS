...
* Также добавлен WidgetKit-виджет для домашнего экрана iOS с быстрым открытием двери. На новых версиях iOS он использует AppIntents, а для совместимости поддерживает открытие через deeplink в приложение.
* Поддерживаются быстрые действия с иконки приложения на домашнем экране iOS: быстрый переход к основным сценариям вроде адресов, камер, оплаты или настроек.
...
* Для событий доступа доступны расширенные сценарии отслеживания: пользователь может настраивать отслеживаемые события и видеть связанные отметки в журнале событий.
* В приложении есть offline/degraded-network режимы: при проблемах с сетью приложение может показывать fallback-интерфейс и использовать локально сохранённые данные по адресам, где это возможно.
* Выбор оператора из списка при первом запуске приложения.
* Поддержка темного режима приложения.
* Приложение локализовано на несколько языков, включая русский, английский, арабский, болгарский, армянский, казахский и узбекский.
* Аналитика приложения построена вокруг Firebase Analytics через внутреннюю абстракцию событий; список публично задокументированных событий находится в `Docs/AnalyticsEvents.md`.
* Для входящих звонков и видео используются не только SIP/linphone-сценарии, но и WebRTC. Для камер и архива используется актуальная playback-инфраструктура приложения, включая SmartYardVideoPlayer, HLS/WHEP-сценарии и обработку live/archive-воспроизведения.
...
* [Firebase Analytics](https://firebase.google.com/docs/analytics) для сбора пользовательской аналитики
* [WebRTC-lib](https://webrtc.org/) для WebRTC-видеосценариев
* [SmartYardVideoPlayer](https://github.com/rosteleset/SmartYard-VideoPlayer-iOS) для воспроизведения live/archive-видео
...
* Онлайн чат через [OnlineChatSdk](https://github.com/rosteleset/OnlineChatSdk-Swift).
