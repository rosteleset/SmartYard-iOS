...
class NotificationsHomeViewModel: BaseViewModel {
    
    private let apiWrapper: APIWrapper
    private let pushNotificationService: PushNotificationService
    private let logoutHelper: LogoutHelper
    private let alertService: AlertService
    private let router: WeakRouter<HomeWebRoute>?
    ...
    init(
        ...
        logoutHelper: LogoutHelper,
        alertService: AlertService,
        routerintercom: WeakRouter<IntercomWebRoute>
    ) {
        self.apiWrapper = apiWrapper
        self.pushNotificationService = pushNotificationService
        self.logoutHelper = logoutHelper
        self.alertService = alertService
        self.router = nil
        self.routerintercom = routerintercom
        self.routerhomepay = nil
    }
        ...
        errorTracker.asDriver()
            ...
            .drive(
                onNext: { [weak self] error in
                    ...
                    self?.routerintercom?.trigger(.alert(title: "Ошибка", message: error.localizedDescription))
        ...
        input.shareUrlTrigger
            .drive(
                onNext: { [weak self] url in
                    ...
                    self?.routerintercom?.trigger(.share(items: [url]))
                    self?.routerhomepay?.trigger(.share(items: [url]))
                }
            )
            .disposed(by: disposeBag)
        ...
        input.backTrigger
            .drive(
                onNext: { [weak self] in
                    ...
                    self?.routerintercom?.trigger(.back)
...
extension NotificationsHomeViewModel {
